...
with st.sidebar:
    with st.spinner():
        ...
        uploaded_file=st.file_uploader(label="Upload Data File", type=["csv","xls" , "xlsx" , "xlsm"], accept_multiple_files=False)
...
try:
    ...
    if uploaded_file is not None:
            ...
            window_size=st.slider(label="Window Size",min_value=0,max_value=200,step=50,value=100)
            ...
            fig, ax = plt.subplots()
            ax.plot(smoothed_df["date"],smoothed_df["production"],color="blue", label=prod_col_name)
            ax.plot(smoothed_df["date"],smoothed_df["production_smoothed"],color="red",label=str(prod_col_name+" SMOOTHED"))
            plt.legend()
            plt.xlabel("Date")
            ...
            ax.plot(T,Q,label="Original Smoothed",color="blue")
            ax.plot(T,Q_exp,label="Exponential",color="red")
            ax.plot(T,Q_hyp,label="Hyperbolic",color="lime")
            ax.plot(T,Q_har,label="Harmonic",color="purple")
            plt.xlabel("Date")
            plt.ylabel("Production")
            plt.legend()
            st.pyplot(fig1)
